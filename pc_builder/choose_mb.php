            ...
            <div class="col-10 mt-3">
                <table class="table table-hover mb-0" style="background-color: #fff">
                    ...
                    <tbody>
                        ...
                        $result = $mysqli->query("
                        SELECT 
                        mb.id_motherboard, CONCAT(m.manufacturer_name, ' ', mb.motherboard_name) as nazwa, s.socket_name as socket, ddr.ddr_type, mb.motherboard_chipset as chipset, mb.motherboard_price as price
                        FROM `motherboard` mb, `manufacturer` m, `ddr`, `socket` s
                        WHERE 
                        mb.manufacturer_id_manufacturer = m.id_manufacturer AND ddr.id_ddr=mb.ddr_id_ddr AND mb.socket_id_socket=s.id_socket
                        GROUP BY mb.id_motherboard;
                        ");
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                echo "<tr>
                                    ...
                                    <td class=\"w-75\">".$row['socket'].", ".$row['ddr_type'].", ".$row['chipset']."</td>
                                    <td>$".$row['price']."</td>
                                    <form action=\"php/scripts/sc_add_mb_build.php\" method=\"POST\">
                                    <input type=\"hidden\" id=\"mb_id\" name=\"mb_id\" value=\"".$row['id_motherboard']."\">
